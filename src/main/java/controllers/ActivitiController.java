...
public class ActivitiController {
	 ...
	 * 
	 * @author：tuzongxun
	 * @Title: getStartFormAndData
	 * @param @param rowStart
	 * @param @param formData1
	 * @param @param firstForm
	 * @param @return
	 * @return StringBuffer
	 * @date Apr 11, 2016 3:58:37 PM
	 * @throws
	 */
	private StringBuffer setFormAndData(String rowStart,
			Map<String, String> formData1, String firstForm) {
		...
		for (int i = 1; i < p.length; i++) {
			...
			if (index1 != -1) {
				...
				String keyName = p[i]
						.substring(index1 + 6, index2 + index1 + 7) + rowStart;
				...
				for (String key : keySet) {
					...
					if (keyString.equals(keyName)) {
						...
						if (value1 != null && !value1.equals("")) {
							...
							// .append(pName
							// +
							// "<input type='text' readonly='readonly' style='background-color:#DEDCDC;margin-top:10px' value='"
							// + value1 + "'></br>");
							firstFormAndData
									.append(pName
											+ "<font  style='background-color:#DEDCDC;margin-top:10px'>"
											+ value1 + "</font></br>");
						}
					}
				}
	...
	public Object hisTask(HttpServletRequest req) {
		...
		if (isLogin) {
			...
			for (HistoricTaskInstance hisTask1 : hisTaskList1) {
				...
				String deployName = "";
				String taskRow = "";
				List<HistoricTaskInstance> hisTaskList11 = historyService
						.createHistoricTaskInstanceQuery()
				...
				try {
					...
					for (HistoricTaskInstance hisTask : hisTaskList11) {
						...
						while (flows.hasNext()) {
							...
							if ("StartEvent".equals(flowElement.getClass()
									.getSimpleName())) {
								rowStart = flowElement.getXmlRowNumber() + "";
								// 获取开始节点填写的数据
								Map<String, String> formData1 = new HashMap<String, String>();
								for (HistoricVariableInstance variableInstance : variables) {
									String varName = variableInstance
											.getVariableName();
									if (varName.contains(rowStart)) {
										formData1.put(varName,
								...
								StringBuffer startForm_Data2 = setFormAndData(
										rowStart, formData1, firstForm);
								startForm_Data.append(startForm_Data2);
							} else if ("UserTask".equals(flowElement.getClass()
									.getSimpleName())
									&& taskName.equals(flowName)) {
						...
						for (HistoricVariableInstance hisVar : hisList) {
							...
							if (varName != null && varName.contains("data")
									&& varName.contains(taskRow)) {
								String var = (String) hisVar.getValue();
								formData2.put(varName, var);
							}
						}
						StringBuffer dataForm1 = setFormAndData(taskRow,
								formData2, taskForm);
						if (dataForm1 != null) {
							startForm_Data
									.append("<hr style='height:2px;border:none;border-top:2px dotted #185598;'></hr>");
